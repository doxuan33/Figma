...
const VIPcheckPage = () => {
  ...
  useEffect(() => {
    // Only clear specific keys if necessary
  ...
  useEffect(() => {
    try {
      ...
      toast.current.show({
        severity: "success",
        summary: "Thành công",
        detail: "Tải thông tin người dùng thành công.",
      });
    } catch (err) {
  ...
  useEffect(() => {
    if (!initialSelectedPackage && packageId) {
      try {
        ...
        if (pkg) {
          ...
          toast.current.show({
            severity: "success",
            summary: "Thành công",
            detail: `Tải thông tin gói/tài nguyên ${pkg.ten_goi || pkg.tieu_de} thành công.`,
          });
        } else {
          ...
          toast.current.show({
            severity: "error",
            summary: "Lỗi",
            detail: "Không tìm thấy gói/tài nguyên.",
          });
        }
      } catch (err) {
        ...
        toast.current.show({
          severity: "error",
          summary: "Lỗi",
          detail: "Không thể tải thông tin gói/tài nguyên.",
        });
      }
      ...
      toast.current.show({
        severity: "error",
        summary: "Lỗi",
        detail: "Không có thông tin gói/tài nguyên.",
      });
    } else {
      setLoadingPackage(false);
    }
  }, [initialSelectedPackage, packageId, toast]);
  ...
  const handlePayment = async () => {
    try {
      ...
      await simulatePayment(2000); // Simulate payment processing
          ...
          <div className="payment-wrapper">
            ...
            <div className="payment-right">
              <div className="payment-form">
                ...
                <Button
                  label={isProcessingPayment ? "Đang xử lý..." : "Xác nhận thanh toán"}
                  className="p-button-success payment-form-submit-button"
